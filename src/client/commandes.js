  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        ...
        const reversedOrders = response.data.commandes.reverse();
        setOrders(reversedOrders);
        setLoading(false); // Arrête le loader une fois les données chargées
      } catch (error) {
        console.error(error);
        setLoading(false); // Arrête le loader en cas d'erreur
      }
    };
    fetchMenus();
  }, [userId, reload]);
    ...
    <Container>
      ...
      >
        ...
        <div style={{ fontSize: 32 }}>
          Nombre total des commandes : {orders.length}
        </div>
      </div>
        ...
        {loading ? (
          <Box
            sx={{
              ...
              alignItems: "center",
              width: "100%",
              height: "300px", // Ajuste la hauteur pour rendre visible le loader
            }}
          >
            <CircularProgress /> {/* Le loader */}
          </Box>
        ) : orders.length > 0 ? (
          orders.map((menu, index) => (
            <Card
